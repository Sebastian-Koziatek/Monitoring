...
Poniższy schemat przedstawia kompletny cykl życia logów - od aplikacji aż po wizualizację:

```
┌─────────────┐
│  Aplikacja  │ → zapisuje logi do /var/log/app.log
└─────────────┘
       ↓
┌─────────────┐
│  Promtail   │ → czyta plik, dodaje etykiety
└─────────────┘
       ↓ (push przez HTTP)
┌─────────────┐
│    Loki     │ → przechowuje logi z indeksami
└─────────────┘
       ↓ (query przez LogQL)
┌─────────────┐
│   Grafana   │ → wizualizuje: wykresy, tabele, alerty
└─────────────┘
```

**Kluczowe elementy przepływu:**

1. **Aplikacja** - generuje logi i zapisuje je do plików (np. `/var/log/app.log`, `/var/log/syslog`)
2. **Promtail** - agent monitorujący pliki logów, dodający etykiety (labels) i wysyłający dane do Lokiego
3. **Loki** - serwer przechowujący logi z indeksami opartymi na etykietach
4. **Grafana** - interfejs użytkownika umożliwiający zapytania LogQL i tworzenie wizualizacji
