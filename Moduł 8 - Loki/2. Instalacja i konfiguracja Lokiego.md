...
> **⚠️ Wersja Lokiego:** Ta konfiguracja jest poprawna i przetestowana dla **Loki 2.9.3** (wersja z tej instrukcji). Uwaga na dwa pola, które są w niej **przestarzałe (deprecated)**:
> - `max_transfer_retries: 0` — **zostaw**. Przy włączonym WAL musi być `0`; jego brak spowoduje błąd walidacji (`max_transfer_retries has to be 0 when WAL is enabled`).
> - `shared_store: filesystem` (w `storage_config.boltdb_shipper` oraz w `compactor`) — działa w 2.9.x, ale **zostało usunięte w Loki ≥ 2.10**. Jeśli w przyszłości zaktualizujesz Lokiego do 2.10+, usuń oba wystąpienia `shared_store` (nowe wersje wykrywają magazyn automatycznie), inaczej Loki nie wstanie.
...
- **WorkingDirectory**: Katalog roboczy procesu — **KRYTYCZNE**. Loki zapisuje w katalogu roboczym plik-marker `shutdown-requested.txt`. Jeśli katalog roboczy to miejsce, do którego użytkownik `loki` nie ma prawa zapisu (np. domyślny katalog, z którego uruchomiono komendę), ingester zgłosi błąd `failed to check ingester shutdown marker: stat shutdown-requested.txt: permission denied` i **usługa nigdy nie wstanie**. Ustawienie `WorkingDirectory=/var/lib/loki` (należy do `loki`) rozwiązuje ten problem.
...
> **⚠️ Najczęstsza przyczyna „Loki nie działa":** Loki **musi** działać jako użytkownik `loki` (przez systemd), a nie z Twojego konta. Jeśli uruchomisz binarkę ręcznie ze swojego katalogu domowego, dostaniesz `mkdir /var/lib/loki/index/uploader: permission denied` (bo katalog należy do `loki`) albo `shutdown-requested.txt: permission denied`. Zawsze uruchamiaj Lokiego przez `sudo systemctl start loki`, nie ręcznie.
...
> **ℹ️ To normalne:** Przez pierwsze ~15–20 sekund po starcie `/ready` zwraca `Ingester not ready: waiting for 15s after being ready`. To celowe opóźnienie startowe ingestera, **nie błąd** — odczekaj chwilę i powtórz `curl`. Dopiero gdy zwróci samo `ready`, Loki jest gotowy przyjmować logi.
...
# Katalog na plik pozycji (positions.yaml) — musi istnieć, inaczej Promtail nie zapisze offsetów
...
# Zapamiętywanie pozycji w plikach (offset do którego już wysłano logi)
positions:
...
scrape_configs:
  ...
  - job_name: audit
    ...
    pipeline_stages:
      # Stage 1: Wyciągnij typ zdarzenia (występuje w KAŻDEJ linii auditd)
      - regex:
          expression: '^type=(?P<audit_type>\w+)'

      # Stage 2: Wyciągnij audit_key OSOBNYM regexem (występuje tylko w części linii)
      - regex:
          expression: 'key="(?P<audit_key>[^"]+)"'

      # Stage 3: Tworzenie etykiet z wyparsowanych wartości
      - labels:
          audit_type:
          audit_key:
```

> **⚠️ Dlaczego DWA osobne regexy, a nie jeden?** W prawdziwych logach auditd pole `key="..."` występuje **tylko w części linii** (na typowym serwerze ~20–25%: głównie w `type=SYSCALL`). Typy `PROCTITLE`, `PATH`, `CWD`, `SOCKADDR`, `EXECVE` **nie mają** pola `key=`. Gdyby użyć jednego regexu wymagającego `key="..."` (jak w starszych wersjach tej instrukcji), to dla ~78% linii cały regex by nie pasował i **nie nadałby nawet etykiety `audit_type`** — a wtedy zapytania w stylu `{job="audit", audit_type="EXECVE"}` zwracałyby pustkę (bo linie EXECVE nie mają `key=`). Rozbijając na dwa niezależne stage'e: `audit_type` trafia na **każdą** linię (100%), a `audit_key` doklejamy tam, gdzie faktycznie istnieje. Zweryfikowane na żywych logach: pokrycie `audit_type` wzrasta z ~22% do 100%.
...
# Powinno zwrócić realne typy, np.: ["SYSCALL","PROCTITLE","PATH","CWD","SOCKADDR","EXECVE","USER_ACCT","USER_CMD", ...]
...
| `audit_type` | Dynamiczne (regex) | `SYSCALL`, `PROCTITLE`, `PATH`, `CWD`, `SOCKADDR`, `EXECVE`, `USER_ACCT`, `USER_CMD` |
...
**Najczęstsze konkretne błędy i ich przyczyny:**

| Błąd w `journalctl -u loki` | Przyczyna | Rozwiązanie |
|-----------------------------|-----------|-------------|
| `mkdir /var/lib/loki/index/uploader: permission denied` | Loki działa jako niewłaściwy użytkownik (Twój, nie `loki`) **lub** pominięto `chown -R loki:loki /var/lib/loki` (Krok 5) | Uruchamiaj przez `systemctl` (unit ma `User=loki`), wykonaj `chown` z Kroku 5 |
| `failed to check ingester shutdown marker: stat shutdown-requested.txt: permission denied` | Katalog roboczy procesu jest niezapisywalny dla `loki` (uruchomiono ręcznie z cudzego katalogu, bez `WorkingDirectory`) | Dodaj `WorkingDirectory=/var/lib/loki` do unitu (Krok 7) i startuj przez systemd |
| `max_transfer_retries has to be 0 when WAL is enabled` | Usunięto `max_transfer_retries: 0` przy włączonym WAL | Przywróć `max_transfer_retries: 0` w sekcji `ingester` |
| `field shared_store not found` (Loki ≥ 2.10) | Użyto konfiguracji 2.9.x na nowszym Lokim | Usuń oba `shared_store` (patrz nota o wersji przy konfiguracji) |
| `bind: address already in use` | Port 3100 zajęty (np. druga instancja Lokiego) | `sudo ss -tlnp \| grep 3100`, zatrzymaj kolidujący proces |

**Złota zasada:** nie uruchamiaj binarki `loki` ręcznie ze swojego konta — zawsze `sudo systemctl start loki`. Ręczny start jako Twój użytkownik prawie zawsze kończy się `permission denied` na `/var/lib/loki`.
