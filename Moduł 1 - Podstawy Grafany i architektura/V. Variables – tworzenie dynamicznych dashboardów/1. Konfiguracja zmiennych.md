...
2. Zmienne w zapytaniach są zastępowane przez wybrane wartości. Grafana używa składni `$nazwa` lub `${nazwa}`. Na przykład, jeśli masz zmienną `host`, możesz użyć jej w zapytaniu PromQL:
```promql
